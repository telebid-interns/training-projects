<?php
    ...
    */
    ...
    function add_map ($atts) {
        try {
            assert_user(is_array($atts), "Shortcode doesn't contain any arguments");
            assert_user(array_key_exists('key', $atts), "API key must be specified");
            assert_user(array_key_exists('width', $atts), "Map width must be specified");
            assert_user(array_key_exists('height', $atts), "Map height must be specified");
            assert_user(array_key_exists('id', $atts), "DOM id must be specified");

            if (!array_key_exists('units', $atts)) {
                $atts['units'] = 'px';
            }

            assert_user(is_numeric($atts['width']), "Width must be a number");
            assert_user(is_numeric($atts['height']), "Height must be a number");

            assert_user(!isset($GLOBALS['key']) || $GLOBALS['key'] === $atts['key'], "Cannot use multiple API keys");
